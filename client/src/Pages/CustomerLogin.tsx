...
const auth = useAuth();
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || "Login failed,please check details again if they are correct.");
    }
  };
    ...
    <StyledWrapper className="styler">
      <form onSubmit={handleLogin} className="form_container">
        ...
        <div className="title_container">
          ...
          <span className="subtitle">We are glad to have you in our store, create an account or login and enjoy the experience.</span>
        </div>
        ...
        <div className="mt-4">
          Don't have an account? <Link to="/customer/register" className="text-blue-500 underline">Sign up </Link>
        </div>
      </form>
    </StyledWrapper>
  );
